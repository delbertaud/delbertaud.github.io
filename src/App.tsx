...
export default function App() {
  ...
  // Handle URL hash routing (e.g. /#resume or /#ai-consultant)
  useEffect(() => {
    const handleHashChange = () => {
      const hash = window.location.hash.replace('#', '');
      if (['services', 'ai-consultant', 'calculator', 'portfolio', 'resume', 'contact'].includes(hash)) {
        setActiveTab(hash);
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans selection:bg-cyan-500 selection:text-slate-950 flex flex-col justify-between">
      <div>
        ...
        <main>
          ...
          {activeTab === 'ai-consultant' && <AIConsultantBot setActiveTab={handleTabChange} />}
